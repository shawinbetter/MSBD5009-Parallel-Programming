// ==============================================================================
// 
//  Write your code in "gensupermer_cuda.cpp" not this file. You may modify
...
typedef int T_read_len;         // max read length = MAX_INT
typedef int T_read_count;       // max number of reads = MAX_INT
typedef /**/int T_CSR_capacity;
typedef unsigned int T_minimizer;
...
public:
    ...
    void append(CSR<T_data, T_attr> new_data) {
        // if (new_data.get_raw_data() == _data) {
        //     // Append itself. Use a new object for further steps.
        //     // 如果是append(CSR<T_data, T_attr> new_data)，则不可能出现此情况，会调用复制构造函数创建临时对象。
        //     new_data = CSR(*this);
        // }
    ...
    T_CSR_capacity capacity() const{
        return _capacity;
    }
    T_CSR_capacity size() const{
        return _size;
    }
    T_CSR_count items() const{
        return _item_count;
    }
    short dtype() const{
...
static void GenerateSupermer_seq(vector<string> reads, int k, int p, _out_ vector<string> &supermers) { // UPDATE 2.1
    for (string read: reads) {
...
inline bool kmer_filter(T_minimizer minimizer, int kmer_selection_ratio=25) {
    return minimizer % 101 <= kmer_selection_ratio;
}
...
namespace MY_UTILS {
    ...
    static const int FILETYPE_FASTA = 1;
    static const int FILETYPE_FASTQ = 2;
    
    static char is_ACTGN(char c) {
        switch (c) {
    ...
    static inline bool ends_with(const char *str, const char *pattern) {
        int len = strlen(str), len_p = strlen(pattern);
        return len>=len_p && strcmp(str+len-len_p, pattern)==0;
    }
    
    static void StripString(string &s) {
    ...
    static int _CheckReadFileType(const char *filename) {
        // 检查read文件类型, 1 for fasta, 2 for fastq, 0 for not supported
        int len = strlen(filename);
        if (ends_with(filename, ".fa"))       return FILETYPE_FASTA;
        if (ends_with(filename, ".fasta"))    return FILETYPE_FASTA;
        if (ends_with(filename, ".fa.gz"))    return FILETYPE_FASTA;
        if (ends_with(filename, ".fasta.gz")) return FILETYPE_FASTA;
        if (ends_with(filename, ".fna"))      return FILETYPE_FASTA;
        if (ends_with(filename, ".fna.gz"))   return FILETYPE_FASTA;
        if (ends_with(filename, ".fq"))       return FILETYPE_FASTQ;
        if (ends_with(filename, ".fastq"))    return FILETYPE_FASTQ;
        if (ends_with(filename, ".fq.gz"))    return FILETYPE_FASTQ;
        if (ends_with(filename, ".fastq.gz")) return FILETYPE_FASTQ;
        return 0;
    }
}


static void SaveSupermers(string &output_path, vector<string> &supermers) {
